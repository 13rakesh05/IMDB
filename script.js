...
searchInput.addEventListener("input", (e) => {
  ...
  if (e.target.value.length > 2) {
    // setting the page number to one for new results
    pageNumber = 1;
    console.log("searchresult ", searchInputtoString(e.target.value));
...
function addSearchElements() {
  ...
  displaySearchedText.innerHTML = `<span class="searched-text" style="color:white" >Results for</span>
  ...
  searchList.forEach((element) => {
    ...
    searchContainer.innerHTML += ` <div class="card-container col-lg-3 col-md-4 col-sm-6 ">
                                <div class="card text-white mb-4 shadow-sm bg-dark ">
                                    <div>
                                        <a href="./moviePage.html?i=${imdbID}">
...
"Response": "True"
  ...
  let togggleClass = "";
  // checking if its already added to favourite or not
  let addedToFav = (() => {
    if (favList.some((row) => row.includes(imdbID))) {
      togggleClass = "added-to-fav";
  ...
  movieDetails.innerHTML = `
    <div class="row g-0 movie-details-container">
        <div class="col-md-4" id="movie-poster">
            ...
            <div class="like-container d-flex justify-content-end position-absolute end-0 top-0 mx-1 my-1">
                <div class="like d-flex justify-content-end position-absolute end-0 top-0 mx-1 my-1 ${togggleClass}"  data-list-active="${addedToFav}" style="">
...
function displayfavoriteList() {
  ...
  totalFavourites = list.length;
  console.log("list", list.length);
  favMsgContainer.innerHTML = `<span class="searched-text">Total Favourites &nbsp:&nbsp</span>
                                 <span class="search-result">${totalFavourites}</span>`;
  ...
  list
    ...
    .forEach((element) => {
      ...
      let imdbID = element.imdbID;
      let togggleClass = "";
      // checking if its already added to favourite list or not
      let addedToFav = (() => {
        if (favList.some((row) => row.includes(imdbID))) {
          togggleClass = "added-to-fav";
      ...
      favListContainer.innerHTML += ` <div class="card-container col-lg-3 col-md-4 col-sm-6 ">
                                        ...
                                        <div>
                                         ...
                                         " onerror="this.onerror=null; this.src='./assets/images/No_Image_Available.jpg'" data-holder-rendered="true "></a>
                                            <div class="like-container d-flex justify-content-end position-absolute end-0 top-0 mx-1 my-1">
                                             <div class="like d-flex justify-content-end position-absolute end-0 top-0 mx-1 my-1"  id="deleteFav">
                                              <i class="fas fa-trash-alt icon trash-icon" id="deleteIcon" onclick="event,deleteFromFavPage('${imdbID}')"></i>
                                            </div>
                                         </div>
                                      </div>
                                      <div class="card-body ">
...
// delete movie from favourite movie list
function deleteFromFavPage(imdbID) {
  ...
  totalFavourites = totalFavourites - 1;
  favMsgContainer.innerHTML = `<span class="searched-text text-center">Total Favorites &nbsp:&nbsp</span>
                                <span class="search-result">${totalFavourites}</span>`;
}
